...
const getParams = (iteration) => {
  ...
  const prize = toWei(faker.finance.amount({ min: 8, max: 16 }))
  const ticketPrice = toWei(faker.finance.amount({ min: 0.01, max: 0.05 }))
...
async function main() {
  ...
  for (let i = 1; i <= iteration; i++) {
    await createLottery(getParams(iteration * i))
  }
  ...
  fs.writeFile('./artifacts/contractAddress.json', address, 'utf8', function (error) {
    if (error) {
      console.log(error)
      throw error
    }
